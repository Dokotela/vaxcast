...
void processLines(List<String> lines) async {
  ...
  for (var skip = 1; skip < lines.length; skip++) {
    ...
    if (row[2] != 'DOB') {
      ...
      await File('./lib/ImmDS/r5/individual_cases/${row[0]}.json')
          .writeAsString(jsonEncode(caseBundle));
      await File('./lib/ImmDS/r5/individual_responses/${row[0]}.json')
          .writeAsString(jsonEncode(responseBundle));
      caseBundle.entry.add(caseEntry(responseParameters));
      await File('./lib/ImmDS/r5/full_cases/${row[0]}.json')
          .writeAsString(jsonEncode(caseBundle));
    }
  }
  await File('./lib/ImmDS/r5/complete_lists/HealthyCasesPatient.json')
      .writeAsString(jsonEncode(patientBundle));
  await File('./lib/ImmDS/r5/complete_lists/HealthyCasesImmunization.json')
      .writeAsString(jsonEncode(immunizationBundle));
  await File('./lib/ImmDS/r5/complete_lists/HealthyCasesEvaluation.json')
      .writeAsString(jsonEncode(evaluationBundle));
  await File('./lib/ImmDS/r5/complete_lists/HealthyCasesRecommendation.json')
