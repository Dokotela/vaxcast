...
class Skippable {
  ...
  bool checkSkipDate(SeriesDose seriesDose) {
    var curTargetSkip = seriesDose.conditionalSkip;
    return curTargetSkip == null
        ? false
        : correctContext(curTargetSkip[0])
            ? isSkippable(curTargetSkip[0])
            : curTargetSkip.length == 1
                ? false
                : correctContext(curTargetSkip[1])
                    ? isSkippable(curTargetSkip[1])
                    : false;
  }

  bool correctContext(ConditionalSkip oldSkip) =>
      (oldSkip.context == 'Both' || oldSkip.context == context);
