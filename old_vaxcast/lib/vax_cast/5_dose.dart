...
class Dose {
  ...
  void evaluateValidAgeDose(
    ...
    setAllowableIntervals(seriesDose, pastDoses, this);
  ...
  Tuple2<bool, EvalReason> isAllowableInterval(
    ...
    if (allowable.fromPrevious == FromPrevious.yes) {
      pastIndex = pastDoses.indexOf(dose) - 1;
  ...
  void evalLiveVirusConflict(
    ...
    pastDoses.forEach((dose) {
      var index = liveList.indexWhere((liveVaccine) =>
          liveVaccine.dateGiven == dose.dateGiven &&
          liveVaccine.cvx == dose.cvx);
      if (index != -1) {
