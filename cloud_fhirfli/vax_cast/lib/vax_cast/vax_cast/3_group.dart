...
class Group {
  ...
  void retainScorableSeries() =>
      vaxSeries.retainWhere((series) => series.scorableSeries);
  ...
  void getScoreableNumbers() {
    for (final series in vaxSeries) {
      ...
      numScorableSeries += series.scorableSeries == null
          ? 0
          : series.scorableSeries
              ? 1
              : 0;
  ...
  void getHighestNumValidDoses(VaxSeries series) {
    var count = 0;
    series.targetDoses
        .forEach((dose) => count += dose == TargetStatus.satisfied ? 1 : 0);
